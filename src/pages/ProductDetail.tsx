...
const ProductDetail = () => {
  ...
  console.log('PRODUCT_DETAIL_IMAGE', id, product?.image);
  ...
  const currentPrice = (product.price * bulkPricing[selectedQuantity].multiplier).toFixed(2);
    ...
    // Get the specific size details (e.g. { quantity: "1G", multiplier: 1 })
    // using the currently selected index
    const sizeOption = bulkPricing[selectedQuantity];

    // Add to Global Context
    // This function inside CartContext will handle saving it and showing the Toast
    addToCart(product, sizeOption, amount);
  };
